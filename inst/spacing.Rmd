...
Above, we compare unary `+` with and without a space; tidyverse
approves of option 1 but not option 2.  This is correct IMO: unary and
binary operators are fundamentally different.  TeX recognises this and
typesets $+1$ differently from $1+1$.  It is in fact quite difficult
to convince TeX to typeset copy such as the chess "win, draw, loss"
notation [something like "$\mathord{+}3\ \mathord{=}4\ \mathord{-}5$",
which would mean 3 wins, 4 draws and 5 losses.  The difficulty is
persuading TeX to treat the equals sign and the minus sign as unary
operators].  So what happens if we want to multiply by negative number?
